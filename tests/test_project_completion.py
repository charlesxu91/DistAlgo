...
class ProjectCompletionTest(unittest.TestCase):
    ...
    def test_deployment_manifests_include_kuberay_and_prometheus(self):
        ...
        minio_manifest = Path("deploy/kubernetes/minio.yaml").read_text(encoding="utf-8")
        minio_smoke = Path("scripts/remote_minio_k3s_smoke.sh").read_text(encoding="utf-8")
        ...
        self.assertIn("kind: Deployment", minio_manifest)
        self.assertIn("distalgo-minio", minio_manifest)
        self.assertIn("kind: Job", minio_smoke)
        self.assertIn("DISTALGO_MINIO_K3S_SMOKE=passed", minio_smoke)
